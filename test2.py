...
class RadioSimulatorApp(ctk.CTk):
    ...
    def on_closing(self):
        """Logs the activity and closes the app"""
        if self.logged_user_name:
            # Only log if a user was actually logged in
            self.log_activity("Logged out (System Closed)")
        
        # Stop any running timers to prevent background errors
        if self.save_timer:
    ...
    def log_activity(self, action_description):
        """Standardized logging for the system"""
        conn = connect_db()
        if conn:
            try:
                cursor = conn.cursor()
    ...
    def show_dashboard(self, selection):
        ...
        if selection == "home":
            ...
            ctk.CTkLabel(header_frame, text="Network Performance Dashboard", font=("Helvetica", 28, "bold"), text_color="#2c3e50").pack(side="left")
            ...
            ax1.axhspan(0, 200, color='#ff7675', alpha=0.1); ax1.axhspan(200, 600, color='#ffeaa7', alpha=0.1); ax1.axhspan(600, 1000, color='#55efc4', alpha=0.1)
            ax1.plot(db_data['days'], db_data['avg_freqs'], marker='o', color='#0984e3', linewidth=3, markersize=8, markerfacecolor='white', markeredgewidth=2)
            ...
            ax2 = fig_p.add_subplot(111); counts = db_data['status_counts']
            if sum(counts) > 0:
                ax2.pie(counts, colors=['#00b894', '#fdcb6e', '#d63031'], startangle=90, wedgeprops={'width': 0.4, 'edgecolor': 'white'})
                uptime = (counts[0]/sum(counts)*100) if sum(counts)>0 else 0
                ax2.text(0, 0, f"{int(uptime)}%\nUptime", ha='center', va='center', fontweight='bold', fontsize=12, color='#2d3436')
            ...
            create_legend_item("Stable (> 600 MHz)", "#00b894"); create_legend_item("Caution (200-600 MHz)", "#fdcb6e"); create_legend_item("Critical (< 200 MHz)", "#d63031")
    ...
    def load_radio_sim(self):
        ...
        self.fig = Figure(figsize=(7, 2), dpi=100); self.ax = self.fig.add_subplot(111); self.ax.set_facecolor("black")
        self.ax.tick_params(colors='#00FF00', labelsize=8); self.x_vals = list(range(50, 1001, 10))
        ...
        self.canvases = []; self.circles = []; labels = ["LOW BAND", "MID BAND", "HIGH BAND"]
        for i in range(3):
            ...
            circ = canv.create_oval(5, 5, 35, 35, fill="grey"); self.canvases.append(canv); self.circles.append(circ)
            ctk.CTkLabel(container, text=labels[i], font=("Arial", 9, "bold"), text_color="#95a5a6").pack()

        ctrl_panel = ctk.CTkFrame(radio_bg, fg_color="#2f3542", corner_radius=15); ctrl_panel.pack(fill="x", padx=30, pady=(10, 20))
        self.mode_switch = ctk.CTkSegmentedButton(ctrl_panel, values=["MANUAL", "AUTO"], command=self.toggle_mode, selected_color="#3498db")
    ...
    def toggle_mode(self, mode):
            ...
            if self.is_auto:
                # Initialize the flag to False until the scan actually FINISHES
                self.save_as_auto = False 
                self.target_f = random.randint(50, 1000)
                self.target_p = random.randint(-50, 10)
                self.run_auto_scan()
    ...
    def run_auto_scan(self):
        ...
        current_p = self.p_knob.get()
        dist_f = self.target_f - current_f
        dist_p = self.target_p - current_p

        if abs(dist_f) < 15 and abs(dist_p) < 3:
            self.f_knob.set(self.target_f)
            self.p_knob.set(self.target_p)
            self.freq_val = self.target_f # Set values directly
            self.power_val = self.target_p
            
            self.update_radio_logic(is_init=True) # Update UI without triggering the 3s timer
            
            # --- THE FIX: Force an immediate save labeled as AUTOMATIC ---
        ...
        # Movement steps
        step_f = 15 if dist_f > 0 else -15
        step_p = 2 if dist_p > 0 else -2

        self.f_knob.set(current_f + step_f)
        self.p_knob.set(current_p + step_p)
        
        # We use is_init=True here to prevent the 3s timer from 
        # firing while the sliders are still sliding.
        self.freq_val = int(current_f + step_f)
        self.power_val = int(current_p + step_p)
        self.update_radio_logic(is_init=True)

        self.after(50, self.run_auto_scan)
    ...
    def force_auto_save(self):
        ...
        if conn:
            try:
                cursor = conn.cursor()
                # Hardcoded "AUTOMATIC" for this specific save event
                sql = "INSERT INTO frequency_logs (username, frequency, power_level, mode) VALUES (%s, %s, %s, %s)"
                values = (self.logged_user_name, self.freq_val, self.power_val, "AUTOMATIC")
    ...
    def save_frequency_log(self):
        # Stop if we just saved via the Auto function
        ...
        if conn:
            try:
                cursor = conn.cursor()
                # If this timer fires, it's because a human moved the slider
                sql = "INSERT INTO frequency_logs (username, frequency, power_level, mode) VALUES (%s, %s, %s, %s)"
                values = (self.logged_user_name, self.freq_val, self.power_val, "MANUAL")
    ...
    def update_radio_logic(self, is_init=False):
        ...
        if self.freq_val < 350:
            ...
            if not is_init: winsound.Beep(1000, 100)
        elif self.freq_val <= 700: self.canvases[1].itemconfig(self.circles[1], fill="yellow")
        else: self.canvases[2].itemconfig(self.circles[2], fill="green")
    ...
    def load_settings(self):
        form_frame = ctk.CTkFrame(self.content_frame, fg_color="white", corner_radius=15); form_frame.pack(pady=30, padx=30, fill="both", expand=True)
        ctk.CTkLabel(form_frame, text="User Account Management", font=("Arial", 24, "bold"), text_color="#2c3e50").pack(pady=20)
        ...
        for field in fields:
            ...
            entry = ctk.CTkEntry(row, width=250, show="*" if field == "Password" else "", fg_color="#ecf0f1", text_color="black")
        ...
        def save_user():
            ...
            try:
                ...
                cursor.execute("INSERT INTO user (username, password, name, email) VALUES (%s,%s,%s,%s)", (u, self.entries['password'].get(), self.entries['name'].get(), self.entries['email'].get()))
                conn.commit()
                # LOG THE CREATION
                self.log_activity(f"Account Created: {u}")
                messagebox.showinfo("Success", "User successfully saved!"); clear_fields()
            except mysql.connector.IntegrityError: messagebox.showerror("Error", "Username already exists!")
            finally: conn.close()
        ...
        def search_user():
            ...
            conn = connect_db(); cursor = conn.cursor()
            cursor.execute("SELECT * FROM user WHERE username=%s", (u,)); res = cursor.fetchone(); conn.close()
            if res:
                clear_fields(); self.entries['username'].insert(0, res[1]); self.entries['password'].insert(0, res[2])
                self.entries['name'].insert(0, res[3]); self.entries['email'].insert(0, res[4])
        ...
        def update_user():
            ...
            conn = connect_db(); cursor = conn.cursor()
            u = self.entries['username'].get()
            cursor.execute("UPDATE user SET password=%s, name=%s, email=%s WHERE username=%s", (self.entries['password'].get(), self.entries['name'].get(), self.entries['email'].get(), u))
            conn.commit()
            if cursor.rowcount > 0:
                # LOG THE UPDATE
                self.log_activity(f"Account Updated: {u}")
    ...
    def show_reports_dashboard(self):
        for widget in self.content_frame.winfo_children(): widget.destroy()
        header_frame = ctk.CTkFrame(self.content_frame, fg_color="transparent"); header_frame.pack(fill="x", padx=20, pady=20)
        ctk.CTkLabel(header_frame, text="System Reports", font=("Arial", 24, "bold"), text_color="black").pack(side="left")
        self.report_type = ctk.CTkSegmentedButton(header_frame, values=["Frequency History", "System Logs"], command=self.refresh_report_table)
    ...
    def refresh_report_table(self, view_type):
        for widget in self.report_container.winfo_children(): widget.destroy()
        conn = connect_db()
        if not conn: return
        cursor = conn.cursor()
        try:
            if view_type == "Frequency History":
                ...
                # UPDATED TO SHOW SYSTEM LOGS (Logins, Logout, CRUD)
                headers = ["USER", "ACTION", "DATE AND TIME"]
                cursor.execute("SELECT username, action, timestamp FROM system_logs ORDER BY timestamp DESC")
            data = cursor.fetchall()
        except: data = []
        finally: conn.close()

        for col, text in enumerate(headers):
            header_cell = ctk.CTkFrame(self.report_container, fg_color="#1e272e", corner_radius=0, border_width=1); header_cell.grid(row=0, column=col, sticky="nsew")
        ...
        for row_idx, row_data in enumerate(data, start=1):
            for col_idx, value in enumerate(row_data):
                ...
                cell = ctk.CTkFrame(self.report_container, fg_color=bg, corner_radius=0, border_width=1, border_color="#57606f"); cell.grid(row=row_idx, column=col_idx, sticky="nsew")
