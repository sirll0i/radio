...
def login():
    ...
    if result:
        ...
        root.destroy()
        open_main_form(logged_user=username)
    else:
...
# ================= MAIN DASHBOARD =================
def open_main_form(logged_user=None):
    main = tk.Tk()
    ...
    def show_dashboard(selection="home"):
        clear_content()
        ...
        if selection == "home":
            ...
            tk.Label(content, text=" history, lagyan ng table", font=("Arial", 20)).pack(pady=50)
    ...
    tk.Button(sidebar, text="  Home", anchor="w", fg="white", bg="#34495e", width=20, command=lambda: show_dashboard("home")).pack(pady=5)
    tk.Button(sidebar, text="  Radio", anchor="w", fg="white", bg="#34495e", width=20, command=lambda: show_dashboard("radio")).pack(pady=5)
    tk.Button(sidebar, text="  Reports", anchor="w", fg="white", bg="#34495e", width=20, command=lambda: show_dashboard("reports")).pack(pady=5)
    tk.Button(sidebar, text="  Settings", anchor="w", fg="white", bg="#34495e", width=20, command=lambda: show_dashboard("settings")).pack(pady=5)
    tk.Button(sidebar, text="  About Us", anchor="w", fg="white", bg="#34495e", width=20, command=lambda: show_dashboard("about")).pack(pady=5)

    # Logout button fixed at the bottom
    logout_btn = tk.Button(sidebar, text="  Logout", anchor="w", fg="white", bg="#34495e", width=20, command=logout)
    logout_btn.pack(side="bottom", fill="x", pady=10)
    # ================= USER + TIME LABEL =================
    
    status_label = tk.Label(sidebar, text="", bg="#2c3e50", fg="white", font=("Arial", 10), justify="left")
    status_label.pack(side="bottom", pady=(50,2))  # above logout button, bottom packing



    def update_status():
        now = datetime.datetime.now().strftime("%d/%m/%Y - %I:%M %p")
        status_label.config(text=f"{logged_user}\n{now}")
        status_label.after(1000, update_status)
